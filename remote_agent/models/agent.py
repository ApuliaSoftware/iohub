...
class Agent(models.Model):
    ...
    def create(self, vals):
        ...
        agent_group = self.env.ref('remote_agent.group_agent_agent')
        if not agent.user.has_group('remote_agent.group_agent_agent'):
            agent_group.write({'users': [(4, agent.user.id)]})
        return agent
...
# Model and UI methods #
    ...
    def adjust_permissions(self):
        ...
        if not self.user.has_group('remote_agent.group_agent_agent'):
            agent_group = self.env.ref('remote_agent.group_agent_agent')
            agent_group.write({'users': [(4, self.user.id)]})
...
# Remote communication methods #
    ...
    def call_http(self, message, timeout=None, silent=False):
        ...
        try:
            ...
            self.sudo().update_state(
                system_name=self.system_name, state='offline',
    ...
    def call_bus(self, message, timeout=None, silent=False):
        ...
        if agent_reply:
            ...
            self.sudo(True).update_state(
                system_name=self.system_name, state='online',
                note='{} reply'.format(message['command']))
...
# RPC Calls from Agent #
    ...
    @api.model
    def update_state(self, system_name=None, state='online',
        ...
        if not agent:
            raise ValidationError(
                _('Agent with name {} not found!').format(system_name))
        state_changed = self.env['remote_agent.agent_state'].search([
        ...
        if (force_create or not state_changed or
            ...
            self.env['remote_agent.agent_state'].create(
                {'agent': agent.id, 'note': note, 'state': state})
        return True
